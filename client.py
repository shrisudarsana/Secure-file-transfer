...
class Client:
    ...
    def server_authentication(self,public_key):
        ...
        * session Key -> bytes
        ...
        print("[CLIENT] Sending initial connection string (512 bytes)")
        self.socket.sendall(initial_conn_string)
        print("[CLIENT] Waiting for length (2 bytes)...")
        length_bytes = self.recv_all(2)
        length = self.bytes_to_int(length_bytes)
        print(f"[CLIENT] Receiving msg_full ({length} bytes)...")
        msg_full = self.recv_all(length)
        msg = msg_full[:32]
        integrity = msg_full[32:]
        ...
        print(f"DEBUG: Na.bytes len={len(Na.bytes)}, Nb len={len(Nb)}")
    ...
    def receive_data(self,seqA,seqB, encryption_key, integrity_key):
        ...
        while True:
            msg = self.get_decrypted_msg(self.recv_all(64),"Bob".encode()+encryption_key+self.int_to_bytes(seqB,32), integrity_key+self.int_to_bytes(seqB,32))
